...
def add_annotations(ds, cfg, instance):
    ...
    for Region in root.iter('Region'):
        ...
        if geo_shape == "Points":
            gos = add_Point(vertex, int(id), text + '_' + geo_shape)
            # ds.GraphicAnnotationSequence[0].ReferencedImageSequence[0].GraphicObjectSequence.append(gos)
            list_gos.append(gos)
            del gos

        if geo_shape == "Rectangle":
            gos = add_Rectangle(vertex, int(id), text + '_' + geo_shape)
            # ds.GraphicAnnotationSequence[0].ReferencedImageSequence[0].GraphicObjectSequence.append(gos)
            list_gos.append(gos)
            del gos

        if geo_shape == "Ellipse":
            gos = add_Ellipse(vertex, int(id), text + '_' + geo_shape)
            # ds.GraphicAnnotationSequence[0].ReferencedImageSequence[0].GraphicObjectSequence.append(gos)
            list_gos.append(gos)
            del gos

        if geo_shape == "Area" or geo_shape == "Polygon":
            gos = add_Area(vertex, int(id), text + '_' + geo_shape)
            # ds.GraphicAnnotationSequence[0].ReferencedImageSequence[0].GraphicObjectSequence.append(gos)
            list_gos.append(gos)
            del gos
